...
```bash
# 원장 파일 경로만 지정하면 prices.journal, scripts/는 자동으로 같은 디렉토리에서 찾음
LEDGER_FILE=~/Library/Mobile\ Documents/com~apple~CloudDocs/ledger/main.journal python3 app.py
```

또는 `run.sh`에 환경변수를 넘겨도 된다:

```bash
LEDGER_FILE=~/ledger/main.journal ./run.sh
```
...
- 최신 시세 정보 + **시세 갱신** 버튼 (`scripts/update-prices.sh` 실행; 스크립트가 `assets:investment` 거래에서 투자 commodity를 자동 탐지)
...
├── run.sh                  시세 갱신 + 서버 실행 단축 스크립트
├── run.command             Finder 더블클릭용 macOS 웹 런처
├── run_silent.command      백그라운드 실행용 macOS 런처
├── raycast/
│   └── open-hledger-web.sh      Raycast용 웹 대시보드 런처
├── launchd/
│   └── com.example.ledger-webui.plist  macOS launchd 자동 시작 설정(템플릿)
├── static/
...
- `run.sh`는 시세 갱신 실패를 무시하고 서버를 실행한다 (pricehist가 없어도 동작).
